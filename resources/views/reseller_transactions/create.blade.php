<x-app-layout>
        ...
        <div class="pc-content">
            <div class="card shadow border-0" style="border-radius: 15px; overflow: hidden;">
                <div class="card-header border-0 text-white d-flex justify-content-between align-items-center"
                    ...
                    <h5 class="mb-0 text-white"><i class="fas fa-plus-circle me-2"></i> {{ $is_retur ? 'Input Retur Reseller' : 'Tambah Transaksi Reseller' }}</h5>
                ...
                <div class="card-body">
                    <form action="{{ route('reseller_transactions.store') }}" method="POST"
                        ...
                        <div class="d-flex justify-content-between align-items-center mb-3">
                            <h6 class="mb-0">Detail Barang <span id="detailPriceMode" class="text-primary fw-normal" style="font-size: 0.85rem;"></span></h6>
                        </div>
    ...
    <script>
        document.addEventListener("DOMContentLoaded", function() {
            ...
            // FUNGSI INI DIUBAH: Menangani filter reseller SEKALIGUS menyembunyikan barang yang sudah dipilih
            function updateDatalistFilter() {
                ...
                // Kumpulkan semua nama barang yang saat ini sudah diketik di semua input tabel
                const usedItems = [];
                document.querySelectorAll('.barang-input').forEach(input => {
                    if (input.value.trim() !== "") {
                ...
                options.forEach(opt => {
                    ...
                    // Kondisi: Jika sesuai dengan reseller DAN belum dipakai di baris lain
                    if ((resellerId === selectedReseller || (!resellerId && !supplierId)) && !isUsed) {
                        opt.disabled = false;
                    } else {
                        opt.disabled = true; // Disabled di datalist akan otomatis menyembunyikan dari dropdown
                    }
                });
            }
            ...
            function addRow() {
                ...
                function updateRow() {
                    ...
                    if (option) {
                        ...
                        if (currentPriceMode === 'jual_lusin') {
                            ...
                            // Untuk jual_potong, grosir, hpp gunakan beli_potong
                    ...
                    // Panggil fungsi filter agar datalist terupdate (barang ini hilang dari opsi lain)
                ...
                removeBtn.addEventListener('click', function() {
                    tr.remove();
                    calculateTotals();
                    // Panggil fungsi filter agar barang yang dihapus muncul lagi di opsi
                    updateDatalistFilter(); 
                });
            }
